...
public class UploadScreenTwo extends AppLayout {
	...
	public UploadScreenTwo(PipelineConfigDao pipelineConfigDao, ChannelConfigDao channelConfigDao, CommonConfigDao commonConfigDao, String pipelineName, String filePath) {
		...
		next.addClickListener(e -> {
			//getUI().get().navigate(UploadScreenOne.class,pipelineName);
		});
		...
		prev.addClickListener(e -> {
			try {
				setContent(new UploadScreenOne(this.pipelineConfigDao,this.commonConfigDao,this.channelConfigDao,this.filePath));
	...
	public void setChannelGrid() {
		...
		Grid.Column<ChannelConfig> instanceColumn =  editChannelConfig.addColumn(ChannelConfig::getInstance).setHeader(new Html(
				"<div style='font-weight:bold;font-size:16px;text-orientation: mixed;background:#f8ca34;color:#4b483f'>Channel</div>"));
		Grid.Column<ChannelConfig> variableColumn = editChannelConfig.addColumn(ChannelConfig::getVariable).setHeader(new Html(
				"<div style='font-weight:bold;font-size:16px;text-orientation: mixed;background:#f8ca34;color:#4b483f'>Variable</div>"));
		Grid.Column<ChannelConfig> valueColumn = editChannelConfig.addColumn(ChannelConfig::getValue).setHeader(new Html(
		...
		save.addClickListener(e -> {
			...
			List<ChannelMaster> masterInstance = this.channelConfigDao.findByPipelineAndChannel(this.pipelineName, this.instance.getValue());
			...
			if(masterInstance == null || masterInstance.isEmpty()) {
				...
				if(insertedInstance != null) {
					...
					if(description.getValue() == null || description.getValue().equals("") || alias.getValue() == null || alias.getValue().equals("") || this.instance.getValue() == null || this.instance.getValue().equals("")) {
	...
	void setDragAndDropGrid() {
		...
		ComponentEventListener<GridDropEvent<EditPipelineConfig>> dropListener = event -> {
		    ...
		    ListDataProvider<EditPipelineConfig> sourceDataProvider = (ListDataProvider<EditPipelineConfig>) dragSource
		            .getDataProvider();
		    List<EditPipelineConfig> sourceItems = new ArrayList<>(
		    ...
		    ListDataProvider<EditPipelineConfig> targetDataProvider = (ListDataProvider<EditPipelineConfig>) targetGrid
		            .getDataProvider();
		    List<EditPipelineConfig> targetItems = new ArrayList<>(
